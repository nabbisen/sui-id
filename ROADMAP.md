...
| R9 | A3.2's self-test harness passes vacuously in CI: `ci-gate.sh`'s HEAD/`GITHUB_SHA` assertion fires inside staged fixture repos, so `expect_gate_fails` cases are satisfied by the precondition error rather than the fixture defect | Certain (observed) | High — the harness that proves other gates do not pass for the wrong reason did exactly that; 8 of 38 assertions were vacuous | First hosted run, `1e59e3d`; caught at the first `expect_gate_passes` | Two fixes specified in the M1a handoff: set `GITHUB_SHA` per staged fixture, and make `expect_gate_fails` verify the failure came from the gate command | Open — blocks M1a closure | architect (design), implementer (fix) |
