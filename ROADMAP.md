...
| 7 | RFC 096-B: federation mutation and session integration | B | M2a seam; 096-A; F01 and F03 built to RFC 102 (Accepted 2026-09-17) | 096 |
...
| R11 | Login-path error propagation is asymmetric: since the U22 conversion (`251629a`) a wrong password for a **known** user propagates a database failure, while unknown-user / disabled / already-locked still swallow it and return 401 | Low likelihood (needs the attacker to induce DB failure at will); certain to exist while the asymmetry does | Medium — a username-enumeration oracle on a live unauthenticated endpoint, observable only during a database failure | Code review; `authn::session.rs:59` helper is `let _ = audit::append(...)` at lines 132/140/156 vs `?` on the converted branch | **Ruled (a), 2026-09-16:** one uniform response. *Corrected the same day on re-reading the code:* `login_post` already maps every login error to the same 401, so the oracle described here is not observable over HTTP. The real residual is different — the error is discarded unlogged, and an audit-write failure silently stops lockout counting while correct-password sign-in still succeeds. Part 1 (measure, log, lock the uniform response with tests) **landed `e39e18b`**; **R11 is closed 2026-09-22:** RFC 102 is Implemented, so a sign-in that cannot be audited does not happen, and the discarded error is logged. Part 2 **ruled 2026-09-16: fail closed** — a sign-in that cannot be audited does not succeed — carried by a new RFC rather than an in-place amendment of RFCs 094/096 (owner, option (b)): [RFC 102](rfcs/done/102-authentication-fails-closed-without-audit.md), **Implemented, closed 2026-09-22.** Every sign-in and step-up path commits its audit event in the same transaction, or does not happen. [`r11-login-failure-accounting.md`](rfcs/handoffs/094-transactional-audit/r11-login-failure-accounting.md) | Will not resolve itself — the three sibling paths are pure deny-and-audit with no mutation, so they stay best-effort by design and the divergence is structural, not transitional | `@nabbisen` |
...
| **B — make the contracts true** | v0.79.0 | **2026-10-13 → 10-24** | RFC 116 stages 1/1b **landed 2026-09-25**; **RFC 118 Accepted 2026-09-25 and dispatched**; then RFC 116 stage 2, RFC 112, RFC 106, then RFC 116 stages 3–4 |
