...
**Open objection, raised by the architect against his own design, 2026-09-24.** The verifier lives in the same tree as the thing it verifies. An agent that writes a ledger entry the gate rejects can weaken the gate, the allowed-signers file or the workflow in the same commit, and CI goes green. The scheme may therefore reduce to *tampering becomes visible in a diff, if someone reads the diff* — which is a weaker claim than this RFC currently makes. **The design review is asked to settle this first**, including whether any construction escapes it, and a recommendation not to build is an acceptable answer.
