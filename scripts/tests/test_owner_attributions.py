...
class GeneratedFiles(unittest.TestCase):
    """RFC 116 stage 3b: a census reads sources, not artefacts. The shipped policy
    excludes .github/workflows/ci.yml, which is generated from a template this
    census does read."""

    SENTENCE = "The owner decided that this lane needs a comment."
    GENERATED = ".github/workflows/ci.yml"
    HEADER = "# GENERATED by scripts/generate-ci-workflow.py from the manifest.\n"
    ...
    def test_a_new_attribution_in_the_template_still_fails(self):
        self.write_pair("Nothing to see here.")
        self.repo.adopt()
        self.write_pair(self.SENTENCE)
        r = self.repo.check()
        self.assertEqual(r.returncode, 1, r.stdout + r.stderr)
    ...
    def test_an_attribution_written_straight_into_the_generated_file_is_not_this_gates_to_catch(self):
        # It is the generator's --check that fails on that (a hand edit makes the
        # file differ from a fresh generation); this census does not read it.
        self.write_pair("Nothing to see here.")
        self.repo.adopt()
        self.repo.write(self.GENERATED, f"{self.HEADER}# {self.SENTENCE}\nname: CI\n")
        r = self.repo.check()
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)

    def test_the_exclusion_cannot_hide_a_hand_written_file(self):
        # Listed as generated, but without the generator's marker: a policy error,
        # not a silent skip.
        self.repo.write(self.GENERATED, f"# {self.SENTENCE}\nname: CI\n")
        r = self.repo.check()
        self.assertEqual(r.returncode, 2, r.stdout + r.stderr)
        self.assertIn("excluded as generated but does not begin with", r.stderr)

    def test_a_file_that_is_not_listed_is_read_however_it_starts(self):
        self.repo.write("doc.md", "# Doc\n")
        self.repo.adopt()
        self.repo.write("other.yml", f"{self.HEADER}# {self.SENTENCE}\n")
        r = self.repo.check()
        self.assertEqual(r.returncode, 1, r.stdout + r.stderr)
        self.assertIn("`other.yml`", r.stdout)

    def test_the_shipped_policy_states_why_the_exclusion_is_safe(self):
        self.assertIn("generate-ci-workflow.py --check", REAL_POLICY)
        self.assertIn("not a gap", REAL_POLICY)
        self.assertEqual(POLICY["generated"], (".github/workflows/ci.yml",))
