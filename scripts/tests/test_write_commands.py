...
class WriteCommandsGate(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.write("contracts/write-commands.toml", rows(SEALED_A01, PLANNED_P01))
    ...
    def inventory(self) -> str:
        return (self.root / "contracts/write-commands.toml").read_text(encoding="utf-8")

    def run_gate(self, *extra: str) -> subprocess.CompletedProcess:
        return subprocess.run(
            [sys.executable, str(CHECKER), "--root", str(self.root),
             "--inventory", "contracts/write-commands.toml", *extra],
            capture_output=True,
            text=True,
        )
    ...
    def test_a_row_naming_a_command_that_does_not_exist_is_caught(self) -> None:
        self.write("contracts/write-commands.toml", rows(SEALED_A01, PLANNED_P01, SEALED_B02))
    ...
    def test_a_wrong_files_path_is_caught(self) -> None:
        ...
        self.write("contracts/write-commands.toml", text)
    ...
    def test_a_planned_row_whose_command_has_been_sealed_is_caught(self) -> None:
        self.write("contracts/write-commands.toml", rows(SEALED_A01, PLANNED_P01.replace('"P01"', '"B02"')))
        self.write("crates/store/src/more.rs", SECOND_RS)
    ...
    def test_a_sealed_row_without_its_derived_fields_is_caught(self) -> None:
        ...
        self.write("contracts/write-commands.toml", "\n".join(lines) + "\n")
    ...
    def test_a_malformed_row_is_caught(self) -> None:
        ...
        self.write("contracts/write-commands.toml", self.inventory().replace('sealed = true\n', ''))
        self.assert_red("missing `sealed`")
        self.write("contracts/write-commands.toml", rows(SEALED_A01, SEALED_A01, PLANNED_P01))
    ...
    def test_a_hand_written_count_in_the_header_is_caught(self) -> None:
        self.write("contracts/write-commands.toml", "# implemented -- code exists (82)\n\n" + self.inventory())
        self.assert_red("(F)", "hand-written count")
        self.write("contracts/write-commands.toml", "# 99 entries in all.\n\n" + self.inventory())
        self.assert_red("(F)", "hand-written count")
    ...
    def test_an_unreadable_inventory_is_exit_two(self) -> None:
        self.write("contracts/write-commands.toml", "this is = not [valid")
        r = self.run_gate()
        self.assertEqual(r.returncode, 2, r.stdout + r.stderr)
    ...
    def test_fix_rewrites_derived_fields_and_nothing_else(self) -> None:
        ...
        self.write("contracts/write-commands.toml", stale)
        self.assert_red("(C)")
        r = self.run_gate("--fix")
        self.assertEqual(r.returncode, 0, r.stderr)
    ...
    def test_fix_does_not_make_a_dead_path_pass(self) -> None:
        self.write("contracts/write-commands.toml", self.inventory().replace("repos/things.rs", "repos/nothing.rs"))
        r = self.run_gate("--fix")
        self.assertEqual(r.returncode, 1, r.stdout + r.stderr)
