...
# Only G01-G09b are expected in [gates]; G10a-G12 use separate mechanisms
# (mdBook, Python, the G12 script) and are intentionally not part of the
# ci-gate.sh dispatcher's command table.
awk -F'\t' '$1 ~ /^G0[1-9][a-z]?$/' "$tmp/rfc-gates-all" | sort >"$tmp/rfc-gates"
