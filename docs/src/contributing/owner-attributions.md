...
`contracts/owner-attribution-baseline.txt`, which lists the attributions that existed
...
of `contracts/owner-attribution-baseline.txt` against the previous commit of a push or
...
3. **Add it to the baseline**, deliberately, in a visible edit to
   `contracts/owner-attribution-baseline.txt` (run
   `python3.14 scripts/check-owner-attributions.py --root . --policy contracts/owner-attributions.toml --update-baseline`
...
- It does not stop a commit that edits the script, the policy or the baseline
  ...
  `contracts/owner-attributions.toml` or `contracts/owner-attribution-baseline.txt` should be
