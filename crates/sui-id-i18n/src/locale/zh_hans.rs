...
pub static STRINGS_ZH_HANS: Strings = Strings {
    ...
    factor_add_password_label: "当前密码",
    factor_add_password_hint: "要添加第一个第二因素，请输入当前密码。",
    factor_add_reauth_failed: "无法确认您的身份。请检查密码后重试。",
    factor_add_reauth_unavailable: "目前无法通过目录验证您的密码，请稍后重试。",
    factor_add_federated_unavailable: "通过外部身份提供方登录的账户暂时无法在此添加第二因素。",
    ...
    reset_done_title: "密码已设置",
    reset_done_lede: "现在可以用新密码登录。此账户的其他会话均已退出。",
    reset_done_lockout_cleared: "此账户曾因多次失败尝试而被拒绝登录，现已解除。",
    reset_done_lockout_watch: "如果不久后再次被拒绝，可能有人正在尝试您账户的密码。请等待几分钟后重试。",
    reset_done_second_factor_before: "需要第二因素的登录已暂停，直到 ",
    reset_done_second_factor_after: "。此次重置不会解除该暂停。",
    ...
    theme_noscript_note: "主题跟随系统设置。",
    empty_users: "暂无用户。",
    empty_users_cta: "创建第一个用户",
    empty_clients: "暂无已注册的应用程序。",
    empty_clients_cta: "注册第一个应用程序",
    ...
    copy_noun_recovery_link: "恢复链接",
    copy_noun_recovery_token: "恢复令牌",
    confirm_recovery_link_title: "要签发账号恢复链接吗？",
    confirm_recovery_link_impact: "将创建一个一次性、有效期 30 分钟的链接。用户打开链接后自行设置密码；您不会看到也不会设置该密码。此用户之前的链接将失效。如果该用户设置了第二因素，登录时仍然需要。",
    confirm_recovery_link_reversibility: "链接交出后无法收回。签发另一个链接，或停用该用户，可使此链接失效。",
    confirm_recovery_link_button: "签发恢复链接",
    recovery_link_handover: "请通过能够确认对方身份的渠道交付链接，例如回拨到已登记的号码，或当面交付。切勿发送到请求者在同一请求中提供的地址或号码。请立即交付：链接 30 分钟后失效。",
    recovery_reason_label: "理由（必填）",
    recovery_reason_placeholder: "例如：已回拨到登记号码核实身份，工单 4711",
    recovery_reason_hint: "将记录在审计日志中。单行，最多 200 个字符。",
    recovery_link_issued_title: "恢复链接已签发",
    recovery_link_issued_once: "链接只显示这一次。请立即复制：离开此页面后将无法再次查看。",
    recovery_link_url_label: "链接",
    recovery_link_url_aria: "恢复链接",
    recovery_link_token_label: "令牌",
    recovery_link_token_hint: "如果邮件或聊天程序去掉了链接的片段（# 之后的部分），用户可以在重置页面粘贴此令牌。",
    recovery_link_token_aria: "恢复令牌",
    recovery_link_expires_label: "有效期至",
    recovery_link_invalidated_label: "已失效的先前链接",
    recovery_link_back: "返回用户",
    recovery_refused_title: "未签发恢复链接",
    recovery_refused_unchanged: "未做任何更改。",
    recovery_refused_reason_required: "必须填写理由。",
    recovery_refused_reason_too_long: "理由过长：最多 200 个字符，且最多 512 字节。",
    recovery_refused_reason_control: "理由不能包含换行或制表符等控制字符。",
    recovery_refused_target_unknown: "没有这个用户。",
    recovery_refused_target_self: "不能为自己的账号签发恢复链接。请在安全页面修改密码，或使用“忘记密码”页面。",
    recovery_refused_target_admin: "无法通过管理面板恢复管理员账号。请让运维人员在主机上运行 sui-id admin issue-recovery-link。",
    recovery_refused_target_non_local: "此账号通过目录服务或上游提供方登录，其密码不在此处管理。",
    recovery_refused_target_disabled: "此用户已被停用。请先启用该账号。",
    recovery_refused_target_deleted: "此用户已被删除。",
    recovery_refused_throttled: "您在一小时内签发的恢复链接已达上限。请稍后再试。",
    recovery_refused_needs_second_factor: "签发恢复链接要求您自己已设置第二因素（验证器应用或通行密钥）。请先在安全页面设置，然后重试。",
};
